...
public sealed class LevelCommands
{
 ...
 public void Level(){
  ...
  var tx=new MText{Location=p.Value+new Vector3d(.40,.05,0),TextHeight=.15,Contents=$"{n.StringResult}  {z.Value:+0.00;-0.00;0.00}",Layer=Layer};ms.AppendEntity(tx);tr.AddNewlyCreatedDBObject(tx,true);
  tr.Commit();}
