...
{
 public sealed record Curve(string Name,List<BeamPostProcessor.SamplePoint> Samples);
 ...
 static BeamPostProcessor.SamplePoint Nearest(List<BeamPostProcessor.SamplePoint> s,double x)=>s.MinBy(p=>Math.Abs(p.GlobalX-x))!;
}
