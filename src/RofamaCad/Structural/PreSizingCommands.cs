...
public sealed class PreSizingCommands
{
 ...
 public void Run(){
  ...
  foreach(ObjectId id in ms){if(tr.GetObject(id,OpenMode.ForRead) is not Entity e)continue;
   ...
   else if(e.Layer=="RF-EST-LAJE"&&e is Polyline s){var ex=s.GeometricExtents;var a=ex.MaxPoint.X-ex.MinPoint.X;var sideB=ex.MaxPoint.Y-ex.MinPoint.Y;var span=Math.Min(a,sideB);var th=Math.Max(.10,span/40.0);Write(e,tr,"ROFAMA_PRESIZE",th,0,"LAJE_HEURISTICA_L40");l++;}
