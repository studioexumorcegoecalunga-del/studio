...
public sealed class IntegratedLoadPathCommands
{
 ...
 static double Dist(Point3d p,Point3d a,Point3d b){var ab=b-a; var ap=p-a;var den=ab.DotProduct(ab);if(den<1e-12)return p.DistanceTo(a);var t=Math.Max(0,Math.Min(1,ap.DotProduct(ab)/den));return p.DistanceTo(a+ab*t);}
}
