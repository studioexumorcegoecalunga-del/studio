...
internal static class EnvelopeInterpolationService
{
 public static BeamPostProcessor.SamplePoint At(IReadOnlyList<BeamPostProcessor.SamplePoint> s,double x){
  ...
  for(int i=0;i<s.Count-1;i++){var a=s[i];var b=s[i+1];if(x<a.GlobalX||x>b.GlobalX)continue;double dx=b.GlobalX-a.GlobalX;if(Math.Abs(dx)<1e-12)return Math.Abs(x-a.GlobalX)<=Math.Abs(x-b.GlobalX)?a:b;double t=(x-a.GlobalX)/dx;return new BeamPostProcessor.SamplePoint(x,a.V+(b.V-a.V)*t,a.M+(b.M-a.M)*t,a.Deflection+(b.Deflection-a.Deflection)*t);}
