...
{
 public static BeamPostProcessor.Sample At(IReadOnlyList<BeamPostProcessor.Sample> s,double x){
  if(s.Count==0)throw new ArgumentException("Amostras vazias.");if(x<=s[0].GlobalX)return s[0];if(x>=s[^1].GlobalX)return s[^1];
  for(int i=0;i<s.Count-1;i++){var a=s[i];var b=s[i+1];if(x<a.GlobalX||x>b.GlobalX)continue;double dx=b.GlobalX-a.GlobalX;if(Math.Abs(dx)<1e-12)return Math.Abs(x-a.GlobalX)<=Math.Abs(x-b.GlobalX)?a:b;double t=(x-a.GlobalX)/dx;return new BeamPostProcessor.Sample(x,a.V+(b.V-a.V)*t,a.M+(b.M-a.M)*t,a.Deflection+(b.Deflection-a.Deflection)*t);}
