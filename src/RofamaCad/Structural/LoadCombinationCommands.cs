using Autodesk.AutoCAD.DatabaseServices;
using Autodesk.AutoCAD.Runtime;
using AcApp=Autodesk.AutoCAD.ApplicationServices.Application;

namespace RofamaCad.Structural;
public sealed class LoadCombinationCommands
{
 [CommandMethod("RFCOMBINACOES")]
 public void Run(){
  var d=AcApp.DocumentManager.MdiActiveDocument;var db=d.Database;using var tr=db.TransactionManager.StartTransaction();var ms=(BlockTableRecord)tr.GetObject(SymbolUtilityServices.GetBlockModelSpaceId(db),OpenMode.ForWrite);int n=0;
  foreach(ObjectId id in ms){if(tr.GetObject(id,OpenMode.ForRead) is not Entity e||e.Layer!="RF-EST-VIGA")continue;
   var slab=Read(e,tr,"ROFAMA_BEAM_TRIBUTARY_LOAD")??0;var wall=Read(e,tr,"ROFAMA_WALL_LOAD")??0;var self=Read(e,tr,"ROFAMA_SELF_WEIGHT")??0;var len=e is Line l?l.Length:0;
   var g=wall*len+self*len;var q=slab;Write(e,tr,"ROFAMA_COMB_SERVICE",g+q,"SERVICO_RASCUNHO");Write(e,tr,"ROFAMA_COMB_DESIGN",g+q,"SEM_COEFICIENTES_NORMATIVOS");n++;
  }tr.Commit();d.Editor.WriteMessage($"\nCombinações-base preparadas em {n} viga(s). Nesta fase os valores NÃO aplicam coeficientes normativos: servem para separar G/Q e preparar o motor ELU/ELS.");
 }
 static double? Read(Entity e,Transaction tr,string k){if(e.ExtensionDictionary.IsNull)return null;var d=(DBDictionary)tr.GetObject(e.ExtensionDictionary,OpenMode.ForRead);if(!d.Contains(k))return null;var a=((Xrecord)tr.GetObject(d.GetAt(k),OpenMode.ForRead)).Data?.AsArray();return a?.Length>0?Convert.ToDouble(a[0].Value):null;}
 static void Write(Entity e,Transaction tr,string k,double v,string tag){e.UpgradeOpen();if(e.ExtensionDictionary.IsNull)e.CreateExtensionDictionary();var d=(DBDictionary)tr.GetObject(e.ExtensionDictionary,OpenMode.ForWrite);Xrecord x;if(d.Contains(k))x=(Xrecord)tr.GetObject(d.GetAt(k),OpenMode.ForWrite);else{x=new Xrecord();d.SetAt(k,x);tr.AddNewlyCreatedDBObject(x,true);}x.Data=new ResultBuffer(new TypedValue((int)DxfCode.Real,v),new TypedValue((int)DxfCode.Text,tag));}
}
