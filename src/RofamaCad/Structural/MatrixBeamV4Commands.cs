...
public sealed class MatrixBeamV4Commands
{
 [CommandMethod("RFMATRIZVIGA4")]
 public void Run(){
  var d=AcApp.DocumentManager.MdiActiveDocument;var db=d.Database;using var tr=db.TransactionManager.StartTransaction();var mat=MaterialCommands.Read(db,tr);if(!mat.HasValue){d.Editor.WriteMessage("\nDefina RFMATERIAL.");return;}var ms=(BlockTableRecord)tr.GetObject(SymbolUtilityServices.GetBlockModelSpaceId(db),OpenMode.ForWrite);int ok=0,fail=0;
  foreach(ObjectId id in ms){if(tr.GetObject(id,OpenMode.ForRead) is not Line b||b.Layer!="RF-EST-VIGA"||b.ExtensionDictionary.IsNull)continue;try{var s=Arr(b,tr,"ROFAMA_ANALYTICAL_SPANS");var q=Arr(b,tr,"ROFAMA_SPAN_LOADS");var bc=BC(b,tr);if(s==null||q==null||bc==null||s.Length!=q.Length||bc.Value.V.Length!=s.Length+1)continue;var sec=Pair(b,tr,"ROFAMA_PRESIZE");double bw=sec?.A??.15,h=sec?.B??.40,I=bw*Math.Pow(h,3)/12,E=mat.Value.E*1000;var rr=FrameBeamSolver.Solve(s,s.Select(_=>E*I).ToArray(),q,bc.Value.V,bc.Value.R);Write(b,tr,rr);ok++;}catch{fail++;}}
  tr.Commit();d.Editor.WriteMessage($"\nSolver V4: {ok} analisada(s), {fail} falha(s). Agora considera deslocamento vertical/rotação e apoios simples, engastados ou livres.");
 }
