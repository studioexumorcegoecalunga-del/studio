...
public sealed class MatrixBeamV2Commands
{
 ...
 public void Run(){
  ...
  foreach(ObjectId id in ms){if(tr.GetObject(id,OpenMode.ForRead) is not Line b||b.Layer!="RF-EST-VIGA"||b.ExtensionDictionary.IsNull)continue;var spans=Spans(b,tr);if(spans==null||spans.Length==0)continue;var sec=Pair(b,tr,"ROFAMA_PRESIZE");double bw=sec?.A??.15,h=sec?.B??.40,I=bw*Math.Pow(h,3)/12.0,E=mat.Value.E*1000.0;var total=Read(b,tr,"ROFAMA_BEAM_TOTAL_TRIBUTARY")??0,w=b.Length>0?total/b.Length:0;var ei=spans.Select(_=>E*I).ToArray();var ws=spans.Select(_=>w).ToArray();var r=BeamStiffnessSolver.Solve(spans,ei,ws);Write(b,tr,r,mat.Value.Fck,mat.Value.E);n++;}
